...
export default function Navbar() {
  ...
  const setLang = (lng) => {
    ...
    window.localStorage.setItem('dbm-lang', lng);
  };
  ...
  const navLinks = [
    ...
    { href: isHome ? '#commodities' : '/#commodities', label: t('nav.commodities') },
  ...
  return (
    <motion.header
      ...
      data-testid="navbar"
    >
      <div className="dbm-container flex items-center justify-between">
        <Link
          to="/"
          data-testid="nav-logo"
          className="flex items-center gap-3"
          aria-label="PT. Dira Baraka Mulia"
        >
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-full flex items-center justify-center text-white font-bold text-sm"
              style={{ background: 'hsl(168 69% 28%)' }}>
              DBM
            </div>
            <span className="hidden md:block font-semibold text-sm" style={{ fontFamily: 'Playfair Display, serif' }}>
              Dira Baraka Mulia
            </span>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            ...
            className="hidden md:inline-flex dbm-btn dbm-btn-primary !py-3 !px-5 !text-xs"
          >
            WhatsApp
          </a>
      ...
      {open && (
        ...
        >
          <div className="dbm-container py-6 flex flex-col gap-4">
            {navLinks.map((l) =>
              l.route ? (
