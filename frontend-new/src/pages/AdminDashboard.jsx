        ...
        <aside className="w-64 hidden md:flex flex-col bg-[hsl(var(--primary))] text-white fixed inset-y-0 left-0 z-40">
          <div className="p-6 border-b border-white/10">
            <div className="bg-white p-2 inline-block">
              <img
                src="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 40 40'><rect width='40' height='40' rx='8' fill='%231a7a6a'/><text x='50%' y='50%' dominant-baseline='central' text-anchor='middle' font-family='serif' font-size='14' font-weight='bold' fill='%23c9a227'>DBM</text></svg>"
                alt="GMA"
                className="h-10 w-auto object-contain"
              />
            </div>
            <div className="text-[10px] uppercase tracking-widest opacity-60 mt-3">
              {t("admin.title")}
            </div>
          </div>
        ...
        <div className="flex-1 md:ml-64">
          ...
          <div className="bg-background border-b border-border p-4 md:p-6 flex items-center justify-between">
            ...
            <div className="md:hidden flex gap-2">
              <button
                onClick={() => setTab("blog")}
                className={`p-2 ${tab === "blog" ? "bg-[hsl(var(--accent))] text-white" : ""}`}
              >
                <FileText size={16} />
              </button>
              <button
                onClick={() => setTab("contacts")}
                className={`p-2 ${tab === "contacts" ? "bg-[hsl(var(--accent))] text-white" : ""}`}
              >
                <Inbox size={16} />
              </button>
      ...
      <div className="bg-background border border-border overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-[hsl(var(--surface-alt))]">
            <tr>
              <th className="text-left p-4 font-semibold uppercase text-xs tracking-wider">
                Title (EN)
              </th>
              <th className="text-left p-4 font-semibold uppercase text-xs tracking-wider">
                Category
              ...
              <th className="text-right p-4 font-semibold uppercase text-xs tracking-wider">
          ...
          <tbody>
            {posts.map((p) => (
              <tr key={p.id} className="border-t border-border">
                ...
                <td className="p-4">
                  <span
                    className={`text-[10px] uppercase tracking-widest px-2 py-1 ${p.published ? "bg-green-500/15 text-green-700 dark:text-green-400" : "bg-yellow-500/15 text-yellow-700 dark:text-yellow-400"}`}
                  >
                ...
                <td className="p-4 text-right">
                  <button
                    ...
                    className="p-2 hover:text-[hsl(var(--accent))]"
                  >
                    <Pencil size={14} />
                  </button>
                  <button
                    ...
                    className="p-2 hover:text-[hsl(var(--destructive))]"
                  >
                    <Trash2 size={14} />
                  </button>
                </td>
...
function BlogEditor({ initial, onClose, onSaved }) {
  ...
  return (
    ...
    >
      <div
        className="bg-background border border-border max-w-3xl w-full my-10"
      ...
      >
        ...
        <form
          onSubmit={save}
          className="p-6 grid md:grid-cols-2 gap-5 max-h-[70vh] overflow-y-auto"
        >
          <Field
            label={t("admin.blog.titleId")}
            name="title_id"
            value={form.title_id}
            onChange={onChg}
            required
          ...
          <div className="md:col-span-2 flex gap-3 pt-2 border-t border-border">
            <button
              ...
              className="dbm-btn dbm-btn-primary disabled:opacity-60"
            >
              {t("admin.blog.save")}
            </button>
            <button
...
function ContactsManager() {
  ...
  const openContact = async (contact) => {
    if (!contact.read) {
      await markAsRead(contact.id, { silent: true });
      setViewing({ ...contact, read: true });
      return;
    }
    setViewing(contact);
  };
  ...
  return (
    <div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.map((c) => (
                <tr
                  key={c.id}
                  className={`border-t border-border ${!c.read ? "bg-[hsl(var(--accent))]/5" : ""}`}
                >
                  ...
                  <td className="p-4 text-right">
                    {!c.read && (
                      <button
                        ...
                        className="p-2 hover:text-[hsl(var(--accent))] disabled:opacity-50"
                        title="Mark as read"
                        aria-label={`Mark ${c.subject} as read`}
                      >
                        {markingId === c.id ? "…" : "✓"}
                      </button>
                    )}
                    <button
                      ...
                      data-testid={`admin-contact-view-${c.id}`}
                      className="p-2 hover:text-[hsl(var(--accent))]"
                    >
                      <Eye size={14} />
                    </button>
      ...
      {viewing && (
        ...
        >
          <div
            className="bg-background border border-border max-w-xl w-full p-6 md:p-8"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="pt-4 border-t border-border whitespace-pre-wrap leading-relaxed">
                {viewing.message}
              </div>
