...
export default function AdminLogin() {
  ...
  const [form, setForm] = useState({ email: 'admin@dira.co.id', password: '' });
      ...
      <section className="min-h-screen flex items-center pt-24 pb-20 bg-[hsl(var(--surface))]">
        <div className="max-w-md w-full mx-auto px-6">
          <div className="bg-background border border-border p-8 md:p-10">
            <div className="dbm-overline mb-3">{t('admin.title')}</div>
            ...
            <form onSubmit={onSubmit} className="space-y-5">
              ...
              {err && <div className="text-sm text-[hsl(var(--destructive))]">{err}</div>}
              <button
                type="submit"
                disabled={loading}
                className="dbm-btn dbm-btn-primary w-full justify-center disabled:opacity-60"
              >
